...
// BuildRealizationPrompt constructs the exact string submitted to InferenceService for generative realization.
func BuildRealizationPrompt(opts output.RealizationOptions, content string, slots []output.Slot) string {
	tone := opts.Tone
	if tone == "" {
		tone = "professional"
	}
	...
	intentSection := ""
	if opts.Intent != "" && strings.ToLower(opts.Intent) != "inform" {
		intentSection = fmt.Sprintf("\n[Communicative Intent]: %s", opts.Intent)
	}
