...
func main() {
	prompt := `You are IDUN, an intelligent AI assistant communicating directly with the user.
...
{"intent":"communicative","status":"mocked"}

[Realized Natural Output]:`

	h1 := sha256.Sum256([]byte(prompt))
	promptRef := hex.EncodeToString(h1[:])
	
	raw := fmt.Sprintf("inf|%s|%s|%s|%f|%d|%s",
		"qwen2.5:1.5b", promptRef, "text",
		0.000000, 0, "standard",
	)
	h2 := sha256.Sum256([]byte(raw))
	cacheKey := "inference/cache/" + hex.EncodeToString(h2[:])

	fmt.Printf("PromptRef: %s\n", promptRef)
	fmt.Printf("Raw Cache String: %s\n", raw)
